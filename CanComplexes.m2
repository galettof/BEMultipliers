...
Lcomplex = method()
Lcomplex(ZZ,Matrix) :=  (b,phi) -> (F:=source phi;
L=new MutableList;
...
Lcomplex(ZZ,Ideal) := (b,I) -> (Lcomplex(b,gens I))

Lcomplex(ZZ,Ring) := (b,R) -> (Lcomplex(b,vars R))
...
Kcomplex = Method()
Kcomplex(ZZ,Matrix) :=  (b,phi) -> (F:= source phi;
    K=new MutableList;
...
Kcomplex(ZZ,Ideal) := (b,I) -> (Kcomplex(b,gens I))

Kcomplex(ZZ,Ring) := (b,R) -> (Kcomplex(b,vars R))
...
--basis list for just the standard Eagon-Northcott
ENBlist = (a,b,M) -> (P:={toList(1..rank source M)};
    G:=target M;
    genENBlist(a,b,P,G)
    )
...
ENO=chainComplex(flatten {exteriorPower(g,M)*(transpose genENprojection(g,0,P,target M)),delete(null,reverse toList ENo)});
return ENO;
)
...
return removeZeroTrailingTerms chainComplex((toList(C)));
)
